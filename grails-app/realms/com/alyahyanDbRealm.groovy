...
class alyahyanDbRealm {
    ...
    def authenticate(authToken) {
        ...
        // exception.
        def user = alyahyanUser.findByUsername(username)
        if (!user) {
        ...
        // in the database.
        def account = new SimpleAccount(username, user.passwordHash, "alyahyanDbRealm")
    ...
    def hasRole(principal, roleName) {
        def roles = alyahyanUser.withCriteria {
            roles {
                eq("name", roleName)
            }
    ...
    def hasAllRoles(principal, roles) {
        def r = alyahyanUser.withCriteria {
            roles {
                'in'("name", roles)
            }
            eq("username", principal)
    ...
    def isPermitted(principal, requiredPermission) {
        ...
        def user = alyahyanUser.findByUsername(principal)
        ...
        def results = alyahyanUser.executeQuery("select distinct p from alyahyanUser as user join user.roles as role join role.permissions as p where user.username = '$principal'")
